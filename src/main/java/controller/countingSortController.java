...
public class countingSortController {
    ...
    public void startOnAction(ActionEvent actionEvent) {
        ...
        // ---------------------------------- Repito pero ahora con el array de contador

        counterView.getColumns().clear();

        int[] counterArray = Elementary.getCount();

        //Convertir array a una lista de Strings para que se vea en fila
        List<String> row2 = new ArrayList<>();
        for (int i = 0; i < counterArray.length; i++)
            row2.add(String.valueOf(counterArray[i]));

        //Una columna para cada valor del array
        ObservableList<List<String>> dataCounter = FXCollections.observableArrayList();
        dataCounter.add(row2);
        ...
        for (int i = 0; i < counterArray.length; i++) {
            ...
            TableColumn<List<String>, String> column = new TableColumn<>("[" + i + "]");
            column.setCellValueFactory(indexValue -> new SimpleStringProperty(indexValue.getValue().get(colIndex)));
            counterView.getColumns().add(column); //Añade las columnas al tableview
        }//end for

        counterView.setItems(dataCounter);
    ...
    private void showCounter(){
        ...
        List<String> row = new ArrayList<>();
        for (int i = 0; i < counterArray.length; i++)
            row.add(String.valueOf(counterArray[i]));
